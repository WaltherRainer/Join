...
/**
 * Renders a dataset-defined icon into the given element if it is not already rendered.
 *
 * Resolves the icon factory from `ICONS`, skips unknown icons and elements that
 * already contain an SVG, then injects the generated SVG markup and applies
 * accessibility attributes.
 *
 * @function renderIconInto
 * @param {HTMLElement} el - Target element containing `data-icon` and optional size data.
 * @returns {void}
 */
function renderIconInto(el) {
  const fn = resolveIconFactory(el);
  if (!fn || el.querySelector("svg")) return;

  el.insertAdjacentHTML("beforeend", fn(readIconSizeArgs(el.dataset)));
  applyIconA11y(el);
}

/**
 * Resolves the icon render function from the global `ICONS` registry.
 *
 * Logs a warning and returns `null` if the icon name is unknown.
 *
 * @function resolveIconFactory
 * @param {HTMLElement} el - Element containing the `data-icon` attribute.
 * @returns {(args?:object)=>string|null} Icon factory function or null.
 */
function resolveIconFactory(el) {
...
/**
 * Applies accessibility attributes to the first SVG found inside the element.
 *
 * @function applyIconA11y
 * @param {HTMLElement} el - Element that contains the injected SVG.
 * @returns {void}
 */
function applyIconA11y(el) {
